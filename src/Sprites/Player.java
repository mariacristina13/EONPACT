...
public class Player extends Sprite {
	...
	public boolean pushLeft(Map tile) {
		boolean touchingRight = getX() + 30 <= tile.getX() + tile.getWidth() && // Player gone past the tiles right
																				// boundary.
				getX() + 30 >= tile.getX() + tile.getWidth() - Constants.PLAYER_SPEED;// Is the player close enough to
																						// the right edge.
	...
	public boolean pushRight(Map tile) {
		boolean touchingLeft = getX() - 20 + getWidth() >= tile.getX() && // Players right side past the logs left side.
				getX() - 20 + getWidth() <= tile.getX() + Constants.PLAYER_SPEED;// Players right side close to logs
																					// left edge.
